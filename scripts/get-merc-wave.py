class getmercatorwave:
    def __init__(self, tstart:str, tend:str):
        ...
        self.logger.info(f"""
        ============================
        Initialized Mercator Wave downloader.
        TSTART: {self.tstart}
        TEND  : {self.tend}
        ============================
        """)
    ...
    def _setup_logger(self):
        ...
        logdir  = datetime.strptime(self.tstart, "%Y%m%d").strftime("/home/cawofcst_dev/devmetmar/logs/download/mercatorwave/%Y/%m")
        logname = f"{logdir}/mercatorwave_{self.tstart}_{self.tend}_{datetime.utcnow().strftime('%Y%m%d_%H%M%S')}.log"
        ...
        # )
        logger = logging.getLogger("MERCATORWAVEDownloader")
    ...
    def load_yaml(self, config="/home/cawofcst_dev/devmetmar/cawo-anal-202412/scripts/config.yml"):
        ...
        with open(config, "r") as f:
            parsed_config = yaml.safe_load(f)['mercatorwave']
        self.logger.info("Loaded MERCATOR WAVE configuration:\n%s", pformat(parsed_config))
    ...
    def get_data(self, **kwargs):
        ...
        if datetime.strptime(self.tstart, "%Y%m%d") < datetime(2023,5,1):
        ...
        target = f"mercatorwave_{self.tstart}_{self.tend}.nc"
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="""
        MERCATOR WAVE Downloader. This script requires config.yaml file storing the following parameters:\n
        - dataset-id\n
        - variables\n
        - area for subsetting\n
        - file format\n
    ...
    getmercatorwave(args.tstart, args.tend).get_data()